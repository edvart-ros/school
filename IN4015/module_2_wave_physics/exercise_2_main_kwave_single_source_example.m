...
number_of_sensors = 16; % <------- CHANGE NUMBER OF ELEMENTS HERE 
...
for rx = 1:channel_data.N_elements
    z_dist = z_pixels-z_element_position(rx);
    x_dist = x_pixels-x_element_position(rx);
    dist = sqrt(z_dist.^2 + x_dist.^2);
    receive_delay(:,:,rx) = dist/1500;
...
psf_x_loc = 0.0615/1000; % Find the x-location of the point scatter in m <------- UPDATE THIS LINE
psf_z_loc = 23.561/1000; % FInd the z-location of the point scatter in m <------- UPDATE THIS LINE
